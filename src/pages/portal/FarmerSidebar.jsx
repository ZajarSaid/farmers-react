import { Link, NavLink } from 'react-router-dom'
import { useAuth } from '../../context/AuthContext.jsx'

const NAV_ITEMS = [
  { section: 'Core', label: 'Dashboard', to: '/me', icon: 'speedometer2', tone: 'cyan', end: true },
  { section: 'Apps', label: 'History', to: '/me/history', icon: 'clock-history', tone: 'indigo' },
  { section: 'Apps', label: 'Market Prices', to: '/me/prices', icon: 'currency-exchange', tone: 'violet' },
  { section: 'Apps', label: 'Register Farm', to: '/me/register-farm', icon: 'pencil-square', tone: 'green' },
  { section: 'Account', label: 'Profile', to: '/me/profile', icon: 'person-fill', tone: 'purple' },
]

function FarmerSidebar({ open, onClose }) {
  const { logout } = useAuth()
  const sections = [...new Set(NAV_ITEMS.map((item) => item.section))]

  return (
    <>
      <div className={`sidebar-overlay ${open ? 'sidebar-overlay--open' : ''}`} onClick={onClose} />
      <aside className={`sidebar ${open ? 'sidebar--open' : ''}`}>
        <div className="sidebar-brand">
          <span className="sidebar-logo">
            <img src="/tz_gov_logo.png" alt="Tanzania Government logo" />
          </span>
          <span className="sidebar-brand-text">Farmers Registry</span>
        </div>

        <nav className="sidebar-nav">
          {sections.map((section) => (
            <div key={section}>
              <div className="sidebar-label">{section}</div>
              {NAV_ITEMS.filter((item) => item.section === section).map((item) => (
                <NavLink
                  key={item.to}
                  to={item.to}
                  end={Boolean(item.end)}
                  className={({ isActive }) => `sidebar-link ${isActive ? 'sidebar-link--active' : ''}`}
                  onClick={onClose}
                >
                  <i className={`bi bi-${item.icon} sidebar-icon sidebar-icon--${item.tone}`}></i>
                  <span>{item.label}</span>
                </NavLink>
              ))}
            </div>
          ))}
        </nav>

        <div className="sidebar-footer">
          <Link to="/" className="sidebar-link">
            <i className="bi bi-globe2 sidebar-icon sidebar-icon--teal"></i>
            <span>View Website</span>
          </Link>
          <button type="button" className="sidebar-link sidebar-logout" onClick={logout}>
            <i className="bi bi-box-arrow-right sidebar-icon sidebar-icon--red"></i>
            <span>Logout</span>
          </button>
        </div>
      </aside>
    </>
  )
}

export default FarmerSidebar